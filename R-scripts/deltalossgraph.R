...
raw<-read.csv("highandlow.csv", sep=",") #Reads the data in each comma
...
p<- p + labs(title="")
p<- p + xlab("Middlebox to Reciever Link Capacity")
...
p <- p + theme(axis.line.x = element_line(color = "black"), axis.line.y = element_line(color="black"), text=element_text(size=25, family = "Times"),axis.title.x=element_text(margin = margin(30,0,0,0)))
...
p
ggsave(filename = "highandlowloss.pdf", plot = last_plot(), width=210,height=297,units="mm")